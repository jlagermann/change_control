...
class SteelHeadCrawler(Application):
    ...
    def archive_running_config(self, device_ip=None, running_config=None, date_string= None, latest_file=None):
        ...
        """
        new_latest = './logs/' + device_ip + '-running-' + date_string + '-latest.log'
        last_latest = None
        if latest_file:
            temp = latest_file
    ...
    def diff_configs(self,logger=None, previous_filename=None, latest_filename=None, device_ip=None):
        if previous_filename is None:
            logger.critical('Cannot find a previous config to diff against.')
            return None
        else:
            logger.info('diff ' + previous_filename + ' vs. ' + latest_filename)

            if os.path.exists(previous_filename) and os.path.exists(latest_filename):
                previous_text = open(previous_filename, "U").readlines()
                latest_text = open(latest_filename, "U").readlines()
                previous_date_match = re.search(r'\d{8}-\d{6}', previous_filename)
                latest_date_match = re.search(r'\d{8}-\d{6}', latest_filename)

                if self.options.html:
                    diff = difflib.HtmlDiff().make_file(previous_text,latest_text,previous_filename,latest_filename,
                                                        True)
                    try:
                    ...
                    diff = difflib.context_diff(previous_text, latest_text, previous_filename,latest_filename,
                                                previous_date_match.group(), latest_date_match.group())

                    diff_file = open('./logs/' + device_ip + '-' + previous_date_match.group() + '-' + latest_date_match.group() + '.diff.txt', "w+")
                    try:
                        for line in diff:
    ...
    def process_steelhead(self, prop):
        ...
        if self.options.archive:
            diff_files = self.archive_running_config(device_ip, sh_running, date_string, latest_filename)

        if self.options.diff and diff_files:
            self.diff_configs(logger, *diff_files, device_ip=device_ip)
